...
| `--pgfplots` | off | — | Five normalized timing tables per gate + one memory table; output to stdout. Combined with `--per-qubit`: emits per-target .dat tables aggregated over targets. |
