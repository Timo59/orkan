// test_pauli.c - Unit tests for pauli_apply and related Pauli string functions.
//
...
//   - Mask path:       string → pauli_from_str → pauli_apply / pauli_exp
 ...
 *   - Under test:  pauli_str → pauli_from_str (bitmasks) → pauli_apply → result
 * =====================================================================================================================
 */
...
static void check_pauli_apply(const char *pauli_str, unsigned n_qubits, const cplx_t *psi) {
    ...
    /* Mask path: string → pauli_t → pauli_apply */
    ...
    pauli_apply(&st, P);
    if (!st.data) {
        fprintf(stderr, "check_pauli_apply(): pauli_apply left state data NULL\n");
        goto cleanup;
    }
...
/*
 * =====================================================================================================================
 * Test: Pauli mask construction — single-qubit cases
 * =====================================================================================================================
 */
...
void test_pauli_to_str(void) {
    ...
    /* n=3 */
    {
        pauli_t *P = pauli_from_str("ZYX", 3);
        TEST_ASSERT_NOT_NULL(P);
        char *s = pauli_to_str(P);
        TEST_ASSERT_NOT_NULL(s);
...
void test_pauli_phase(void) {
    ...
    PX = pauli_from_str("X", 1);
    TEST_ASSERT_NOT_NULL(PX);
    PY = pauli_from_str("Y", 1);
    TEST_ASSERT_NOT_NULL(PY);
    PZ = pauli_from_str("Z", 1);
...
/*
 * =====================================================================================================================
 * Test: pauli_from_str — encoding of "ZYX" (qubit 0 = X, qubit 1 = Y, qubit 2 = Z)
 *
 * "ZYX" big-endian: s[0]='Z' -> qubit 2, s[1]='Y' -> qubit 1, s[2]='X' -> qubit 0.
 * Expected masks:
...
void test_pauli_from_str_roundtrip(void) {
    ...
    for (unsigned si = 0; si < nvecs; ++si)
        check_pauli_apply("ZYX", 3, states[si]);
...
void test_pauli_phase_eta3(void) {
    pauli_t *P = pauli_from_str("YYY", 3);
    TEST_ASSERT_NOT_NULL(P);
...
/*
 * =====================================================================================================================
 * Gap 5: pauli_apply — YYY (eta_exp = 3, the only previously untested value)
 *
...
/*
 * =====================================================================================================================
 * Gap 7: pauli_apply — YZ (eta_exp=1 with z-phase from separate qubit)
 *
 ...
 * n_qubits == 0 causes an early return with all masks zero.
 *
...
void test_pauli_mask_invariant(void) {
    ...
    /* ZYX (big-endian): qubit 0=X, qubit 1=Y, qubit 2=Z -> y_mask=bit1=2 */
    {
        pauli_t *P = pauli_from_str("ZYX", 3);
        TEST_ASSERT_NOT_NULL(P);
        TEST_ASSERT_EQUAL_UINT64(2u, P->y_mask);
 ...
 *
 * circ.h documents: "Safe to call with NULL."
...
/*
 * =====================================================================================================================
 * Test: pauli_from_str with n_qubits = 64 — boundary of valid range
 *
 * n=64 is the maximum valid input. The mask construction uses `(uint64_t)1 << j`
 * for j = 0..63; qubit 63 exercises `(uint64_t)1 << 63`, well-defined for uint64_t.
 *
 * Big-endian string: s[0] = qubit 63 = 'X', s[1] = qubit 62 = 'Y', s[2..63] = 'I'.
 * Verify:
...
void test_pauli_from_str_n64(void) {
    char s[65];
    memset(s, 'I', 64);
    s[0]  = 'X';  /* qubit 63 (big-endian s[0]) = X */
    s[1]  = 'Y';  /* qubit 62 (big-endian s[1]) = Y */
    s[64] = '\0';

    pauli_t *P = pauli_from_str(s, 64);
    TEST_ASSERT_NOT_NULL(P);
 ...
 * pauli_exp tests
 *
 * pauli_exp applies U(theta) = exp(-i*theta/2 * P) = c*I - i*s*P, where
 ...
 *   - Under test:  pauli_str → pauli_from_str (bitmasks)  → pauli_exp → result
 *
 * Convention reminder:
...
static void check_pauli_exp(const char *pauli_str, unsigned n_qubits,
    ...
    /* Mask path: string → pauli_t → pauli_exp */
    P = pauli_from_str(pauli_str, (qubit_t)n_qubits);
    ...
    pauli_exp(&st, P, theta);
    if (!st.data) {
        fprintf(stderr, "check_pauli_exp(): pauli_exp left state data NULL\n");
        goto cleanup;
    }
...
/*
 * =====================================================================================================================
 * Test: pauli_exp — Branch 1: Identity Pauli (x_mask=0, z_mask=0)
 *
...
/*
 * =====================================================================================================================
 * Test: pauli_exp — Branch 2: Pure Z-string (x_mask=0, y_mask=0)
 *
...
/*
 * =====================================================================================================================
 * Test: pauli_exp — Branch 3: Non-diagonal (x_mask != 0)
 *
...
void test_pauli_exp_nondiaganal_X(void) {
...
void test_pauli_exp_nondiaganal_Y(void) {
    unsigned nvecs; cplx_t **s = test_mk_states_pure(1, &nvecs);
    for (unsigned ti = 0; ti < N_EXP_THETAS; ++ti)
        for (unsigned si = 0; si < nvecs; ++si)
...
void test_pauli_exp_nondiaganal_XX(void) {
...
void test_pauli_exp_nondiaganal_YY(void) {
    unsigned nvecs; cplx_t **s = test_mk_states_pure(2, &nvecs);
    for (unsigned ti = 0; ti < N_EXP_THETAS; ++ti)
        for (unsigned si = 0; si < nvecs; ++si)
...
void test_pauli_exp_nondiaganal_YYY(void) {
...
void test_pauli_exp_nondiaganal_XYZ(void) {
...
/*
 * =====================================================================================================================
 * Test: pauli_exp — theta=0 is identity for all branches
 *
...
/*
 * =====================================================================================================================
 * Test: pauli_exp — theta=pi recovers -i * pauli_apply
 *
 * At theta=pi: c=0, s=1.  U = -i * P.
 * So pauli_exp(state, P, pi) == -i * P|psi>.
 *
 * Equivalently: pauli_exp output * i must equal pauli_apply output.
 *
...
/*
 * =====================================================================================================================
 * Test: pauli_exp — theta=2*pi gives global -1 (NOT identity)
 *
...
/*
 * =====================================================================================================================
 * Test: pauli_exp — unitarity / norm preservation
 *
...
/*
 * =====================================================================================================================
 * Test: pauli_exp — inverse property: U(theta) * U(-theta) = I
 *
 * Applying pauli_exp twice with +theta then -theta must recover the original state.
...
/*
 * =====================================================================================================================
 * Test: pauli_exp — eigenstate phases
 *
...
/*
 * =====================================================================================================================
 * Test: pauli_exp — theta=4*pi is true identity (contrast with theta=2*pi = -I)
 *
...
/*
 * =====================================================================================================================
 * Test: pauli_exp — consistency with pauli_apply at theta=pi
 *
 * At theta=pi: c=0, s=1.  exp(-i*pi/2 * P) = -i * P.
 * So pauli_exp output equals -i * (pauli_apply output).
...
void test_pauli_exp_consistency_with_apply(void) {
     ...
     *   pauli_exp(psi, P, pi)[k]  ==  -i * (pauli_apply(psi, P))[k]
     *
     * Reference: apply P via pauli_apply, scale by -i, compare against pauli_exp.
     */
...
/*
 * =====================================================================================================================
 * Test: pauli_exp — non-unit-norm state: norm is preserved proportionally
 *
 * pauli_exp is defined for arbitrary pure states; unitarity guarantees that
...
/*
 * =====================================================================================================================
 * Test: pauli_exp — n=4, XZYX (exercises dim=16, split_bit and all branches at max qubits)
 * =====================================================================================================================
 */
...
/*
 * =====================================================================================================================
 * Test: pauli_exp — non-diagonal with split_bit > 0
 *
 * All 18 existing pauli_exp tests that exercise the non-diagonal branch (x_mask != 0)
