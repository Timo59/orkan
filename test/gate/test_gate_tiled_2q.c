// test_gate_tiled_2q.c  - Test harness for two-qubit quantum gates on tiled mixed states. The reference is given by
                        ...
                        for (unsigned cc = 0; cc < dim; ++cc) {
                            for (unsigned rr = 0; rr < dim; ++rr) {
                                ...
                                if (rd > 1e-12 || rd < -1e-12 || id > 1e-12 || id < -1e-12) {
                                    fprintf(stderr, "MISMATCH n=%u q1=%u q2=%u state=%u at (%u,%u): got=%.6e+%.6ei exp=%.6e+%.6ei\n",
                                        nqubits, q1, q2, i, rr, cc, creal(got), cimag(got), creal(exp), cimag(exp));
                                    goto done_check;
                                }
                            }
                        }
