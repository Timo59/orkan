...
void applyOpPauli_blas(state_t* state, const pauliOp_t* op, ptrStr applyPauli) {
    ...
    for (complength_t i = 0; i < op->length; ++i) {                     // Iterate the observable's terms
        ...
        applyPauli(&tmp, op->comps + (i * op->qubits));    // Apply the Pauli string to it
...
void applyOpPauli_blas_omp(state_t* state, const pauliOp_t* op, ptrStr applyPauli) {
    __LAPACK_int N = (__LAPACK_int) state->dim;
    cplx_t* tmpSum = calloc(state->dim, sizeof(cplx_t));                // Temporary vector holding the intermediate sum
# pragma omp parallel default(none) shared(applyPauli, N, op, state, tmpSum)
    {
        state_t tmp;
        stateInitEmpty(&tmp, state->qubits);
...
void applyOpPauli_old(state_t* state, const pauliOp_t* op, ptrStr applyPauli) {
    state_t tmp;                                                        // Temporary state to apply each Pauli string to
    stateInitEmpty(&tmp, state->qubits);
    cplx_t* tmpSum = calloc(state->dim, sizeof(cplx_t));     // Temporary vector holding the intermediate sum
                                                                        // of state vectors
...
void applyOpPauli_omp(state_t* state, const pauliOp_t* op, ptrStr applyPauli) {
    cplx_t *tmpSum = calloc(state->dim, sizeof(cplx_t));                // Temporary vector holding the intermediate sum

# pragma omp parallel default(none) shared(applyPauli, op, state, tmpSum)
    {
        state_t tmp;                                                    // Temporary state to apply each Pauli string to
        stateInitEmpty(&tmp, state->qubits);
...
void applyObsPauli_blas(state_t* state, const pauliObs_t* obs, ptrStr applyPauli) {
    __LAPACK_int N = (__LAPACK_int) state->dim;
    state_t tmp;                                                        // Temporary state to apply each Pauli string to
    stateInitEmpty(&tmp, state->qubits);
    cplx_t* tmpSum = calloc(state->dim, sizeof(cplx_t));                // Temporary vector holding the intermediate sum
    ...
    for (complength_t i = 0; i < obs->length; ++i) {                    // Iterate the observable's terms
        cblas_zcopy(N, state->vec, 1, tmp.vec, 1);                      // Copy input's state vector to tmp
        applyPauli(&tmp, obs->comps + (i * obs->qubits));            // Apply the Pauli string to it
...
void applyObsPauli_blas_omp(state_t* state, const pauliObs_t* obs, ptrStr applyPauli) {
    __LAPACK_int N = (__LAPACK_int) state->dim;
    cplx_t* tmpSum = calloc(state->dim, sizeof(cplx_t));                // Temporary vector holding the intermediate sum

# pragma omp parallel default(none) shared(applyPauli, N, obs, state, tmpSum)
    {
        state_t tmp;
        stateInitEmpty(&tmp, state->qubits);
...
void applyObsPauli_old(state_t* state, const pauliObs_t* obs, ptrStr applyPauli) {
    state_t tmp;                                                        // Temporary state to apply each Pauli string to
    stateInitEmpty(&tmp, state->qubits);
    cplx_t* tmpSum = calloc(state->dim, sizeof(cplx_t));                // Temporary vector holding the intermediate sum
    // of state vectors
...
void applyObsPauli_omp(state_t* state, const pauliObs_t* obs, ptrStr applyPauli) {
    cplx_t* tmpSum = calloc(state->dim, sizeof(cplx_t));                // Temporary vector holding the intermediate sum

# pragma omp parallel default(none) shared(applyPauli, obs, state, tmpSum)
    {
        state_t tmp;                                                    // Temporary state to apply each Pauli string to
        stateInitEmpty(&tmp, state->qubits);
...
void evolvePauliStr_blas(state_t* state, const pauli_t paulistr[], double angle, ptrStr applyPauli) {
    __LAPACK_int N = (__LAPACK_int) state->dim;
    ...
    applyPauli(state, paulistr);                                    // Apply the Pauli string to the input state
    alpha = -I * sin(angle);
...
void evolvePauliStr_blas_omp(state_t* state, const pauli_t paulistr[], double angle, ptrStr applyPauli) {
    cplx_t* tmp = malloc(sizeof(cplx_t) * state->dim);              // Temporary vector holding state vector multiplied
                                                                    // with cos(angle)
...
void evolvePauliStr_old(state_t* state, const pauli_t paulistr[], double angle, ptrStr applyPauli) {
    cplx_t* tmp = malloc(sizeof(cplx_t) * state->dim);          // Temporary vector holding state vector multiplied
                                                                // with cos(angle)
    for (dim_t entry = 0; entry < state->dim; ++entry) {        // Iterate entries of the input's state vector
        tmp[entry] = cos(angle) * state->vec[entry];            // Store it times cos(angle) in tmp
...
void evolvePauliStr_omp(state_t* state, const pauli_t paulistr[], double angle, ptrStr applyPauli) {
    cplx_t* tmp = malloc(sizeof(cplx_t) * state->dim);              // Temporary vector holding state vector multiplied
                                                                        // with cos(angle)
#pragma omp parallel for default(none) shared(angle, state, tmp)
    for (dim_t entry = 0; entry < state->dim; ++entry) {            // Iterate entries of the input's state vector
