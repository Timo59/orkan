...
double* kronecker(const double a[], const double b[], unsigned long long dimA, unsigned long long dimB) {
    unsigned long long rowIndex, colIndex, entry;                    // unsigned integers holding the indices
            ...
            */
            for (unsigned long long k = 0; k < dimB; ++k) {
                ...
                */
                ...
                {
                    rowIndex = dimB * i + k;          // The kronecker product's current
                                                                // row is A's current row times the
                                                                // number of rows in B plus B's
                                                                // current row

                    colIndex = dimB * j + l;          // The kronecker product's current
