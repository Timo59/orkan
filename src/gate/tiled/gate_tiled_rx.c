/**
 * @file gate_tiled_rx.c
 * @brief Rx rotation gate for tiled density matrices
 *
 ...
 *   rho'_00 = c2*rho00 + s2*rho11 + i*cs*(rho01 - rho10)
 *   rho'_11 = s2*rho00 + c2*rho11 - i*cs*(rho01 - rho10)
 *   rho'_01 = i*cs*(rho00 - rho11) + c2*rho01 + s2*rho10
 *   rho'_10 = -i*cs*(rho00 - rho11) + c2*rho10 + s2*rho01
 *
 * Note: i*cs*(complex z) = CMPLX(-cs*cimag(z), cs*creal(z)).
 *
 * Two traversal modes:
            ...
            for (gate_idx_t tc = 0; tc <= tr; ++tc) {
                ...
                for (gate_idx_t br = 0; br < n_base; ++br) {
                    ...
                    for (gate_idx_t bc = 0; bc < n_base; ++bc) {
                        ...
                        cplx_t d01 = v01 - v10;
                        cplx_t ics = CMPLX(-cs * cimag(d01), cs * creal(d01));

                        cplx_t dd  = v00 - v11;
                        cplx_t icd = CMPLX(-cs * cimag(dd), cs * creal(dd));

                        data[c0 + offset_r0] = c2 * v00 + s2 * v11 + ics;
                        data[c1 + offset_r0] =  icd + c2 * v01 + s2 * v10;
                        data[c0 + offset_r1] = -icd + c2 * v10 + s2 * v01;
                        data[c1 + offset_r1] = s2 * v00 + c2 * v11 - ics;
                    }
                }
            }
        }
                ...
                if (tr0 > tc1) {
                    ...
                    for (gate_idx_t lr = 0; lr < TILE_DIM; ++lr) {
                        ...
                        for (gate_idx_t lc = 0; lc < TILE_DIM; ++lc) {
                            ...
                            row00[lc] = c2 * v00 + s2 * v11 + ics;
                            row01[lc] =  icd + c2 * v01 + s2 * v10;
                            row10[lc] = -icd + c2 * v10 + s2 * v01;
                            row11[lc] = s2 * v00 + c2 * v11 - ics;
                        }
                    }
                }
                else if (btr != btc) {
                    ...
                    for (gate_idx_t lr = 0; lr < TILE_DIM; ++lr) {
                        ...
                        for (gate_idx_t lc = 0; lc < TILE_DIM; ++lc) {
                            ...
                            data[idx00 + lc] = c2 * v00 + s2 * v11 + ics;
                            data[idx11 + lc] = s2 * v00 + c2 * v11 - ics;
                            data[idx10 + lc] = -icd + c2 * v10 + s2 * v01;

                            cplx_t new01 = icd + c2 * v01 + s2 * v10;
                    ...
                    for (gate_idx_t lr = 0; lr < TILE_DIM; ++lr) {
                        ...
                        for (gate_idx_t lc = 0; lc <= lr; ++lc) {
                            ...
                            cplx_t n00 = c2 * v00 + s2 * v11 + ics;
                            cplx_t n01 =  icd + c2 * v01 + s2 * v10;
                            cplx_t n10 = -icd + c2 * v10 + s2 * v01;
                            cplx_t n11 = s2 * v00 + c2 * v11 - ics;
