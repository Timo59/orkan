/**
 * @file gate_tiled_tdg.c
 * @brief T-dagger gate for tiled density matrices
 *
 * Tdg = diag(1, e^(-ipi/4)).  Diagonal gate: rho' = Tdg rho Tdg+.
 *
            ...
            for (idx_t btc = 0; btc <= btr; ++btc) {
                ...
                if (btr != btc) {
                    if (tr0 > tc1) {
                         ...
                         *            = conj(rho01) * e^(-ipi/4)
                         *            = stored * (1-i)/sqrt(2)
                         * So: CMPLX((r + im) * M_SQRT1_2, (im - r) * M_SQRT1_2) */
