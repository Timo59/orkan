/**
 * @file gate_tiled_hy.c
 * @brief Hadamard-Y gate for tiled density matrices
 *
 * HY = (1/sqrt(2)) [[1, -1], [1, 1]].  Self-adjoint (HY = HY+), so rho' = HY rho HY.
 *
 * Conjugation on each 2x2 block of the density matrix:
 *   rho'_00 = 0.5 * (rho00 - rho01 - rho10 + rho11)
 *   rho'_01 = 0.5 * (rho00 + rho01 - rho10 - rho11)
 *   rho'_10 = 0.5 * (rho00 - rho01 + rho10 - rho11)
 *   rho'_11 = 0.5 * (rho00 + rho01 + rho10 + rho11)
 *
 * All 4 elements mix -- this is a non-permutation, non-diagonal gate.
 * Structurally identical to H but with a different sign pattern.
 *
 * Two traversal modes:
        ...
        for (idx_t btr = 0; btr < n_base; ++btr) {
            ...
            for (idx_t btc = 0; btc <= btr; ++btc) {
                ...
                if (tr0 > tc1) {
                     ...
                     * The write of n10 to *a10 then overwrite with n10
